...
def test_merge_addresses_does_not_copy_delayed_note_to_package_with_same_address(warehouse_reset, monkeypatch):
    table = hash_table.HashTable(size=4)
    ...
    # New contract: delayed notes are never propagated by merge_addresses
    assert pkg_j.special_note is None
    assert pkg_j.delivery_deadline == datetime.time(9, 15)

def test_merge_addresses_does_not_unify_deadlines_when_only_shared_constraint_is_delayed_note(warehouse_reset, monkeypatch):
    table = hash_table.HashTable(size=4)

    pkg_i = package.Package(
        1, "123 Maple Street", "Springfield", "IL", 62701, "11:00 AM", 2.5, "D, 9:30 AM"
    )
    pkg_j = package.Package(
        2, "123 Maple Street", "Springfield", "IL", 62701, "9:15 AM", 2.5, None
    )
    ...
    # j deadline should not be rewritten because i has D and is skipped as a merge source
    assert pkg_j.delivery_deadline == datetime.time(9, 15)
